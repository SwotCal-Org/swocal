...
const HARDCODED_COUPON_ROWS: CouponRecord[] = [
  {
    id: 'swocal-seed-1',
    google_place_id: '1',
    business_name: 'Bastille Market Coffee',
    business_address: 'Boulevard Richard-Lenoir, Paris',
    business_category: 'Market Cafe',
  ...
  },
  {
    id: 'swocal-seed-2',
    google_place_id: '2',
    business_name: 'Rue Cler Market Stand',
    business_address: 'Rue Cler, 75007 Paris',
    business_category: 'Farmers Market',
  ...
  },
  {
    ...
    google_place_id: '3',
    business_name: 'Marché d’Aligre Gourmet',
    business_address: "Place d'Aligre, 75012 Paris",
    business_category: 'Market Food',
...
export default function CouponsScreen() {
  ...
  const [couponRows] = useState<CouponRecord[]>(HARDCODED_COUPON_ROWS);
            ...
            {active.length === 0 ? (
              ...
              active.map((p) => (
                <CouponRow
                  ...
                  place={p}
                  onPress={() => {
                    if (!p.couponRouteId) return;
                    router.push((`/coupon/${p.couponRouteId}` as never));
                  }}
                />
              ))
            )}
...
function CouponRow({ place, onPress }: { place: Place; onPress: () => void }) {
  const c = place.coupon!;
  const redeemed = c.status === 'redeemed';
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [styles.card, redeemed && styles.cardRedeemed, pressed && { opacity: 0.9 }]}
    >
